...
// protocol but don't match any existing endpoint. We return False as we don't
// actually do anything but leave it to the default handler.
//   Returns:
//   * A boolean telling the caller whether the packet was well formed.
//   * A boolean telling the caller whether it handled the error,
...
func (*protocol) Parse(pkt *stack.PacketBuffer) bool {
	...
	if !ok {
		// Packet is too small. We should report it to
		// ?.Stats().UDP.MalformedPacketsReceived.Increment()
		// except we have no way to find it.
		return false
	}
