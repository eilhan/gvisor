...
func (s *sender) updateMaxPayloadSize(mtu, count int) {
	...
	for seg := s.writeList.Front(); seg != nil; seg = seg.segmentEntry.Next() {
		if seg == s.writeNext {
...
func (s *sender) NextSeg(nextSegHint *segment) (nextSeg, hint *segment, rescueRtx bool) {
	...
	var s4 *segment
	// Step 1.
	for seg := nextSegHint; seg != nil; seg = seg.segmentEntry.Next() {
		...
		if !s.ep.scoreboard.IsSACKED(header.SACKBlock{segSeq, segSeq.Add(1)}) {
			...
			if s.fr.highRxt.LessThan(segSeq) && segSeq.LessThan(s.ep.scoreboard.maxSACKED) {
				...
				if s.ep.scoreboard.IsLost(segSeq) {
					return seg, seg.segmentEntry.Next(), false
				}
	...
	for seg := s.writeNext; seg != nil; seg = seg.segmentEntry.Next() {
...
func (s *sender) maybeSendSegment(seg *segment, limit int, end seqnum.Value) (sent bool) {
	...
	if !s.isAssignedSequenceNumber(seg) {
		...
		if seg.data.Size() != 0 {
			...
			// implementations.
			var nextTooBig bool
			for seg.segmentEntry.Next() != nil && seg.segmentEntry.Next().data.Size() != 0 {
				if seg.data.Size()+seg.segmentEntry.Next().data.Size() > available {
					nextTooBig = true
					break
				}
				seg.data.Append(seg.segmentEntry.Next().data)

				// Consume the segment that we just merged in.
				s.writeList.Remove(seg.segmentEntry.Next())
				s.rcList.Remove(seg.rackSegmentEntry.Next())
			}
...
func (s *sender) handleSACKRecovery(limit int, end seqnum.Value) (dataSent bool) {
	...
	for s.outstanding < s.sndCwnd {
		...
		if !s.isAssignedSequenceNumber(nextSeg) || s.sndNxt.LessThanEq(nextSeg.sequenceNumber) {
			...
			dataSent = true
			s.outstanding++
			s.writeNext = nextSeg.segmentEntry.Next()
			continue
		}
...
func (s *sender) sendData() {
	...
	if s.fr.active && s.ep.sackPermitted {
		...
		for seg := s.writeNext; seg != nil && s.outstanding < s.sndCwnd; seg = seg.segmentEntry.Next() {
			...
			if s.isAssignedSequenceNumber(seg) && s.ep.sackPermitted && s.ep.scoreboard.IsSACKED(seg.sackBlock()) {
				...
				s.writeNext = seg.segmentEntry.Next()
				continue
			}
...
func (s *sender) SetPipe() {
	...
	for s1 := s.writeList.Front(); s1 != nil && s1.data.Size() != 0 && s.isAssignedSequenceNumber(s1); s1 = s1.segmentEntry.Next() {
